    ...
    ui(new Ui::MainWindow)
{
    ...
    QObject::connect(dlg2, &Dialog2::calltime1, this, &MainWindow::on_saveButton_clicked);
...
{
    //从接收区中读取数据
    QByteArray rece_data = serial.readAll();
    emit datatochart(rece_data);
    //从界面中读取以前收到的数据
    QString recv = ui->recvText->toPlainText();
    recv += QString(rece_data);
    //清空以前的显示
    ui->recvText->clear();
    //重新显示
    ui->recvText->append(recv);
}
...
void MainWindow::on_saveButton_clicked()
{
    ...
    if(flag2==0)
    {
        QMessageBox::about(this, "温馨提示", "接收区数据以文本文档格式保存在电脑桌面处。");
    }
    else
    {
        //定时保存的话就不用弹出消息框了
